...
int main(int argc, char *argv[])
{
	...
	Interval init_x0(-0.5 - w, -0.5 + w), init_x1(0.5 - w, 0.5 + w), init_x2(0.26 - w, 0.26 + w), init_u(0); //w=0.01
	...
	string NUM = argv[6];
	string nn_name = "systems_with_networks/reachnn_oscillator_benchmark/nn_test_relu_tanh_"+NUM;
	...
	ofstream result_output("./outputs/reachnn_oscillator_relu_tanh_" + to_string(if_symbo) + ".txt");
	...
	// plot_setting.plot_2D_octagon_MATLAB("reachnn_oscillator_relu_tanh_" + to_string(if_symbo), result);
	...
	cnpy::npy_save("StepReach0_"+NUM+".npy", &StepReach[0], {StepReach.size()}, "w");
	// assert(false);
