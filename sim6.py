...
class Benchmark:
    ...
    def __init__(self, x0=None, x1=None, x2=None, x3=None):
        if x0 is None or x1 is None or x2 is None or x3 is None:
            x0 = np.random.uniform(low=-0.77, high=-0.75, size=1)[0]
            x1 = np.random.uniform(low=-0.45, high=-0.43, size=1)[0]
            x2 = np.random.uniform(low=0.51, high=0.54, size=1)[0]
            x3 = np.random.uniform(low=-0.3, high=-0.28, size=1)[0]
            self.x0 = x0
            self.x1 = x1
            self.x2 = x2
            self.x3 = x3
    ...
    def step(self, action):
        ...
        x1_tmp = self.state[1] + self.deltaT*(-self.state[0]+0.1*np.sin(self.state[2]))
        x2_tmp = self.state[2] + self.deltaT*self.state[3]
        x3_tmp = self.state[3] + self.deltaT*u
        
        self.t = self.t + 1
        # reward = self.design_reward()
        self.state = np.array([x0_tmp, x1_tmp, x2_tmp, x3_tmp])
...
if __name__ == '__main__':
    ...
    for i in range(env.max_iteration):
        control_input = state[2] - 2*state[3]
        # print(control_input)
